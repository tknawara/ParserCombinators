...
public interface Monad<T, MClass extends Monad<?, MClass>> {
	 ...
	 * 
	 * @param value
	 *            the value to lift inside the monad
	 * @return a monad of this value
	 */
	public <B> Monad<B, MClass> pure(B value);
	 ...
	 */
	public <B> Monad<B, MClass> map(Function<T, B> f);
	 ...
	 * 
	 * @param f
	 *            function to use in the mapping
	 * @return a new monad produced by the function
	 */
	public <B> Monad<B, MClass> flatMap(Function<T, Monad<B, MClass>> f);
	 ...
	 * 
	 * @param other
	 *            the action which its result will be returned
	 * @return action that will run both actions and return the result of the
	 *         {@code other} action
	 */
	default public <B> Monad<B, MClass> then(Monad<B, MClass> other) {
	 ...
	 */
	default public <C> Monad<C, MClass> apply(Monad<Function<T, C>, MClass> fM) {
	 ...
	 * 
	 * @param f
	 *            curried function that takes to values and return a new value
	 * @param paramTwo
	 *            the second parameter to apply. the first one is the instance
	 *            itself
	 * @return the result of the function lifted to the monadic world
	 */
	default public <B, C> Monad<C, MClass> liftM2(final Function<T, Function<B, C>> f,
	 ...
	 * 
	 * @param f
	 *            Uncured function that takes to values and return a new value
	 * @param paramTwo
	 *            the second parameter to apply. the first one is the instance
	 *            itself
	 * @return the result of the function lifted to the monadic world
	 */
	default public <B, C> Monad<C, MClass> liftM2(BiFunction<T, B, C> f, Monad<B, MClass> paramTwo) {
	 ...
	 * 
	 * @param mList
	 *            list of monadic instances of type {@code B}
	 * @return a monad of a list of type {@code B}
	 */
	default public <B> Monad<IList<B>, MClass> sequence(final IList<? extends Monad<B, MClass>> mList) {
	 ...
	 * 
	 * @param f
	 *            this function takes a value of type {@code A} and returns a
	 *            monad of {@code B}
	 * @param list
	 *            list of values of type {@code A}
	 * @return m t b
	 * @see {@link #mapM_(Monad)}
	 */
	default public <A, B> Monad<IList<B>, MClass> mapM(final Function<A, Monad<B, MClass>> f, final IList<A> list) {
	 ...
	 * @param list
	 *            list of elements of generic type {@code A}
	 * @return Unit
	 */
	default public <A, B> Unit mapM_(final Function<A, Monad<B, MClass>> f, final IList<A> list) {
		final IList<Monad<B, MClass>> mList = list.map(f);
		this.sequence(mList);
		return Unit.get();
	}
	 ...
	 * 
	 * @param traversable
	 *            iterator of elements of type {@code A}
	 * @param f
	 *            a transformation function from {@code A} to {@code Monad<B>}
	 * @return a monad of the list resulting from evaluating each monadic action
	 *         and collect the results
	 */

	default public <A, B> Monad<IList<B>, MClass> forM(final IList<A> list, final Function<A, Monad<B, MClass>> f) {
	 ...
	 *         and collect the results
	 */
	default public <A, B> Unit forM_(final IList<A> list, final Function<A, Monad<B, MClass>> f) {
