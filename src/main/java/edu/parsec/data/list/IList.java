...
public interface IList<T> extends Iterable<T>, Monad<T, IList<?>> {
     ...
     */
    IList<T> filter(final Predicate<T> p);
     ...
     */
    IList<T> takeWhile(final Predicate<T> p);
    ...
    default IList<T> reverse() {
        return this.foldLeft((IList<T>) new Empty<>(), IList::addFront);
    }
     ...
     */
    IList<T> append(final IList<T> other);
     ...
     */
    static <T> IList<T> of(final Iterable<T> traversable) {
     ...
     *
     * @param traversable the iterator to build the list with.
     * @return a list of elements of the iterator
     */
    static <T> IList<T> of(final Iterator<T> traversable) {
        if (traversable.hasNext()) {
            return new Cons<>(traversable.next(), of(traversable));
        }
        return new Empty<>();
    }
     ...
     */
    <U> U foldLeft(final U zeroElement, final BiFunction<U, T, U> f);

    /**
     * Apply the function successively over the list from right to left
     * using {@code zerElement} as the starting element.
     *
     * @param zerElement element to start the reduction with
     * @param f reducing function
     * @param <U> type of the reduced result
     * @return single value after the successive applications of the function
     */
    default <U> U foldRight(final U zerElement, final BiFunction<T, U, U> f) {
        final IList<T> reverse = this.reverse();
        return reverse.foldLeft(zerElement, (x, y) -> f.apply(y, x));
    }
